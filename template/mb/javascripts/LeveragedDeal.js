var vue = new Vue({
	...
	mounted: function () {
		...
		that.leverDatas.legalNameTxt = get_param('name1')=='USDT'?'RMB':get_param('name1') || '';
	...
	methods: {
		...
		ganggan(){
		    layer.open({
				...
				title: false,
				area: ['100%', 'auto'],
				skin: 'confirm-modal ganggan-borrow',
				shadeClose: true,
				fixed: true,
				offset: 'b',
				closeBtn: 0,
	    ...
	    getgangganVal(val,index){
		    let that=this;
		    if(index!==''){
		        this.gangganIndex=index;
		        setTimeout(function(){
    		        that.gangganIndex='';
    		    },300)
		    }else{
    		    this.gangganBool=true;
    		    setTimeout(function(){
    		        that.gangganBool=false;
    		    },300)
		    }
		    this.selectMult(val);
		},
	    ...
	    jumpSecondContract(){
	      let that = this;
			let symbol=that.leverDatas.currencyName+"/"+that.leverDatas.legalName;
			window.location.href = 'dataMap.html?tradeId='+that.heyueId+'&legal_id=' + that.leverDatas.legalId + '&currency_id=' + that.leverDatas.currencyId + '&symbol=' + symbol;
	    },
		init() {
			var that = this;
			//法币、币种数据请求（左侧内容）
		...
		get_lever_data() {
			...
			initDataTokens({
				...
				if (res.type == "ok") {
					...
					for (i in sellList) {
						arr = [];
				// 		arr[0] = sellList[i].price *that.rmb;
					...
					for (let i in buyList) {
						arr3 = [];
				// 		arr3[0] = buyList[i].price *that.rmb;
							...
							if (msg.type == 'market_depth') {
								if (that.leverDatas.legalId == msg.legal_id && that.leverDatas.currencyId == msg.currency_id) {
									...
									for(let i=0;i<buyIn.length;i++){
									   // buyIn[i][0]=buyIn[i][0]*that.rmb
									   buyIn[i][0]=buyIn[i][0]
									}
									for(let i=0;i<out.length;i++){
									   // out[i][0]=out[i][0]*that.rmb
									   out[i][0]=out[i][0]
									}
									that.sellOut = out;
									that.buyOut = buyIn;
		...
		legalTab(legalid) {
			...
			$('.market-value').text('≈ 0.0000' + that.leverDatas.legalNameTxt);
			$('.bond').text('≈ 0.0000' + that.leverDatas.legalNameTxt);
			$('.transaction-fee').text('≈ 0.0000' +that.leverDatas.legalNameTxt);
		},
		// 币种切换 
		...
		calculation(bond, type, share, muit) {
			...
			var shareNum = that.leverDatas.share;
			...
			if (marketPrice == 'NaN') {
				...
				// $('.market-value').text('≈ ' + marketPrice *that.rmb + " " + that.leverDatas.legalNameTxt);
				$('.market-value').text('≈ ' + marketPrice + " " + that.leverDatas.legalNameTxt);
			}
			if (bonds == "NaN") {
				$('.bond').text('≈ 0.0000' + that.leverDatas.legalNameTxt);
			} else {
				// that.leverDatas.bondTotal = bonds *that.rmb;
				// $('.bond').text('≈ ' + bonds * that.rmb + " " + that.leverDatas.legalNameTxt);
					that.leverDatas.bondTotal = bonds 
				$('.bond').text('≈ ' + bonds  + " " + that.leverDatas.legalNameTxt);
			}
			if (tradeFree == "NaN") {
				$('.transaction-fee').text('≈ 0.0000' + that.leverDatas.legalNameTxt);
			} else {
				// that.leverDatas.transactionTotal = tradeFree *that.rmb;
				// $('.transaction-fee').text('≈ ' + tradeFree *that.rmb + " " + that.leverDatas.legalNameTxt);
				that.leverDatas.transactionTotal = tradeFree ;
				$('.transaction-fee').text('≈ ' + tradeFree  + " " + that.leverDatas.legalNameTxt);
			}
			setTimeout(function () {
				layer_close();
			}, 500)
		...
		linkLine() {
			let that = this;
			let symbol=that.leverDatas.currencyName+"/"+that.leverDatas.legalName;
			window.location.href = 'dataMap.html?heyueId='+that.heyueId+'&legal_id=' + that.leverDatas.legalId + '&currency_id=' + that.leverDatas.currencyId + '&symbol=' + symbol;
