		...
		listAjax(texts) {
			...
			initDataTokens({
				...
				// 	that.totalCount=res.message.change_wallet.usdt_totle+res.message.lever_wallet.usdt_totle+res.message.micro_wallet.usdt_totle+res.message.legal_wallet.usdt_totle;
				    let totalCount=0;
				    let lever_wallet=res.message.lever_wallet.balance;
				    let micro_wallet=res.message.micro_wallet.balance;
				    for(let i=0;i<lever_wallet.length;i++){
    			        if(lever_wallet[i].currency==3){
    			            totalCount+=parseFloat(lever_wallet[i].lever_balance)+parseFloat(lever_wallet[i].lock_lever_balance);
    			            break;
    			        }
    			    }
    			    for(let i=0;i<micro_wallet.length;i++){
    			        if(micro_wallet[i].currency==3){
    			            totalCount+=parseFloat(micro_wallet[i].micro_balance)+parseFloat(micro_wallet[i].lock_micro_balance);
    			            break;
    			        }
    			    }
